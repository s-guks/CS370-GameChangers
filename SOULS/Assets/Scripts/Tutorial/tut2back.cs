...
public class tut2back : MonoBehaviour
{
    ...
    public TutorialManager7 TutorialManager7;

    // Start is called before the first frame update
    void Start()
    {
        TutorialManager7 = GameObject.Find("TutorialManager7").GetComponent<TutorialManager7>();
    ...
    void Update()
    {
        ...
        if(Input.GetMouseButtonDown(0)) { //if user clicks
            if(Physics.Raycast(ray,out hit) && hit.collider.gameObject == gameObject) { //if click on button
                TutorialManager7.previous1(); //trigger event in separate script
            }
        }
    }
}
